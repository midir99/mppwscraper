...
def scrapp_po_poster_url(po_post_url: str) -> str:
    url = po_post_url
    ...
    try:
        po_poster_url = div.img.get("src", "").strip()
    except AttributeError as e:
        logging.exception("Unable to retrieve the po_poster_url from %s", url)
        po_poster_url = ""

    return po_poster_url


def scrap_mpp_data(article) -> dict[str, Any]:
    mpp_data = {}
    try:
        mpp_data["mp_name"] = article.h2.get_text().strip().title()
    except AttributeError as e:
        logging.exception("Unable to get the mp_name from the article: %s", article)
        raise e

    try:
        mpp_data["po_post_url"] = article.a.get("href", "").strip()
    except AttributeError as e:
        logging.exception("Unable to get the po_post_url from the article: %s", article)
        raise e

    try:
        datestr = article.find("span", class_="published").get_text().strip()
    ...
    try:
        mpp_data["po_poster_url"] = scrapp_po_poster_url(mpp_data["po_post_url"])
...
def add_common_data_to_mpp(mpp_: mpp.MissingPersonPoster):
    mpp_.alert_type = mpp.AlertTypeChoices.AMBER.value
