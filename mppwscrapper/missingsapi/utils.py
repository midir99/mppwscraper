...
BASE_URL = "https://extraviados.mx/api/v1"
...
def count_mpps_by_post_url(post_url: str) -> list[dict]:
    ...
    url = f"{BASE_URL}/mpps/?po_post_url={parsed_post_url}"
...
def update_counter(token: str):
    url = f"{BASE_URL}/counter/updated_at/"
    req = urllib.request.Request(url, b"", method="PUT")
    req.add_header("Authorization", f"Token {token}")
    try:
        with urllib.request.urlopen(req) as response:
            if response.status != 200:
                raise exceptions.APIError(
                    f"Update counter endpoint returned an unexpected error code: "
                    f"{response.status}"
                )
            res_body = response.read().decode("UTF-8")
    except urllib.error.HTTPError as e:
